-- INFO: Edit a filesystem using a buffer
    ...
    return astrocore.extend_tbl(opts, {
      ...
      view_options = {
        is_hidden_file = function(name, bufnr)
          ...
          if not git_avail or not dir then
            return is_hidden
          end
          ...
          oil_settings = {
            {
              ...
              callback = function(args)
                vim.b[args.buf].view_activated = false
              end,
            },
            {
              event = "User",
        ...
        if opts.winbar then
          ...
          table.insert(opts.winbar, 1, {
            condition = function(self)
              return status.condition.buffer_matches({ filetype = "^oil$" }, self.bufnr)
            end,
